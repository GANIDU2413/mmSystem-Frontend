...
export default function CertifyMarksheet(props) {     
  ...
  const { authState } = useOktaAuth();
    ...
    const {approvedLevel}=props;     // Approved level for result board conducted courses
              ...
              <tbody>
                {
                  !finalMarksheetList.length >0 ? (
                    ...
                    finalMarksheetList.map((item, index) =>(
                      <tr key={index} className='clickable-row' onClick={()=>{approvedLevel=="AR"?history.push(`/deanFinalMarkSheet/${item.level}/${item.semester}/${item.department}`):approvedLevel=="Dean"?history.push(`/vcFinalMarkSheet/${item.level}/${item.semester}/${item.department}`):null}}>
